"""
Time series visualization using EpiDataset with target/context node filtering.

Analyzes normalization, interpolation, and smoothing effects on case data
by plotting a subset of regions alongside aggregated global series using
EpiDataset for proper region filtering.
"""
...
def plot_interpolation_effects(
    ...
    for idx, method in enumerate(methods):
        interpolated = smooth_series(series_with_gaps, window=1, method="rolling")
        ax = axes[idx]
...
def plot_scale_statistics(
    ...
    per_100k_np = (cases_np * (100000.0 / population.values)).T
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Time series visualization using EpiDataset"
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/train_epifor_filtered.yaml"),
        help="Training config path for dataset loading",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs/reports/timeseries_analysis_filtered"),
    ...
    # Use all nodes as targets and context (EpiDataset will handle filtering by valid_targets)
    ...
    population_da = epi_dataset.dataset.population
    ...
    if "valid_targets" in epi_dataset.dataset:
        ...
        population_da = population_da.isel({REGION_COORD: valid_mask})
    ...
    population = pd.Series(population_da.values, index=regions)

    cases_df = pd.DataFrame(cases_da.values, index=dates, columns=regions)
    cases_per100k_df = per_100k_scaling(cases_df, population)

    global_weighted = compute_global_aggregation(
        cases_per100k_df, population, method="weighted_mean"
    )
