...
| --- | --- | --- |
| `population` | `(region,)` (persisted as `(region, 1)` in Zarr) | Static per administrative region. Sourced from the case metadata and used to normalize rates if needed. |
...
- **Missing Data:** Days with no samples are handled by the Kalman filter's prediction step without an update, naturally increasing the uncertainty (variance) for those periods while maintaining a continuous state estimate.
...
| --- | --- | --- |
| `cases` | `(time, region, 1)` | Canonical case counts aligned to the forecasting horizon. |
| `edar_biomarker_<variant>` | `(time, region)` | Per-variant biomarker intensity per region. Kalman-filtered values; may be interpolated. |
| `edar_biomarker_<variant>_mask` | `(time, region)` | **1.0** if measured today (finite and positive), **0.0** otherwise. Distinguishes actual measurements from Kalman-interpolated values. |
| `edar_biomarker_<variant>_censor` | `(time, region)` | **0** = uncensored, **1** = censored at limit of detection, **2** = missing. Indicates Tobit Kalman censoring status. |
| `edar_biomarker_<variant>_age` | `(time, region)` | Days since last measurement, normalized to **[0, 1]** (max 14 days). **1.0** = no measurement ever (regions without EDAR coverage). |
| `mobility` | `(time, region, region, 1)` | Time-indexed OD flow tensor. Empty mobility inputs are materialized as zeros with matching shapes so downstream code can rely on the dimension. |
| `population` | `(region, 1)` | Static population per region. |
