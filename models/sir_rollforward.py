...
class SIRRollForward(nn.Module):
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized SIRRollForward: gamma={gamma}, dt={dt}, "
    ...
    def forward(
        ...
        """
        Roll forward SIR dynamics from initial states.
        ...
        Returns:
            Dictionary containing:
                ...
                - physics_residual: [batch_size, horizon] L2 residual of SIR dynamics
                - beta_t: [batch_size, horizon] (echoed back for convenience)

        Note:
            The trajectories include the initial state at index 0, so they have
            length horizon+1. This makes indexing intuitive: trajectory[t] is
            the state at time t.
        """
        if beta_t.ndim != 2:
            raise ValueError(
        ...
        for t in range(horizon):
            ...
            # SIR derivatives (before applying dt)
            # dS/dt = -beta * S * I / N
            # dI/dt = beta * S * I / N - gamma * I
            # dR/dt = gamma * I
            ...
            dI_dt = beta_SI_over_N - self.gamma * I_t
            dR_dt = self.gamma * I_t

            # Euler step (raw, before constraints)
            ...
            if self.enforce_mass_conservation:
                ...
                # Scale to maintain S + I + R = N
                scale = population / (total + 1e-8)  # Avoid division by zero
                S_next = S_next * scale
                I_next = I_next * scale
                R_next = R_next * scale

            # Store next states (append to lists - no in-place ops)
            S_list.append(S_next)
            I_list.append(I_next)
            R_list.append(R_next)

            # Compute physics residual on raw step before constraints
            # This measures how well the discrete step satisfied the continuous dynamics
            dI_expected = beta_SI_over_N - self.gamma * I_t
            dI_actual = (I_next_raw - I_t) / self.dt
            physics_residual_list.append((dI_actual - dI_expected) ** 2)

        # Stack lists into tensors [batch_size, horizon+1]
        S_traj = torch.stack(S_list, dim=1)
        I_traj = torch.stack(I_list, dim=1)
        R_traj = torch.stack(R_list, dim=1)
        physics_residual = torch.stack(physics_residual_list, dim=1)
    ...
    def compute_sir_loss(
        ...
        This loss penalizes deviations from the SIR differential equations.
        It can be used as L_sir in the total loss: L_total = w_ww * L_ww + w_hosp * L_hosp + w_sir * L_sir
        ...
        Returns:
            Scalar loss value (mean squared residual across all compartments and time)
        """
        batch_size, horizon = beta_t.shape

        # Extract states at each step (excluding final for derivatives)
        S_t = S_trajectory[:, :-1]  # [batch_size, horizon]
        I_t = I_trajectory[:, :-1]  # [batch_size, horizon]
        R_t = R_trajectory[:, :-1]  # [batch_size, horizon]

        S_next = S_trajectory[:, 1:]  # [batch_size, horizon]
        I_next = I_trajectory[:, 1:]  # [batch_size, horizon]
        R_next = R_trajectory[:, 1:]  # [batch_size, horizon]

        # Expand population for broadcasting
        N = population.unsqueeze(-1)  # [batch_size, 1]

        # Compute actual derivatives from trajectory
        dS_actual = (S_next - S_t) / self.dt
        dI_actual = (I_next - I_t) / self.dt
        dR_actual = (R_next - R_t) / self.dt

        # Compute expected derivatives from SIR equations
        beta_SI_over_N = beta_t * S_t * I_t / N

        dS_expected = -beta_SI_over_N
        dI_expected = beta_SI_over_N - self.gamma * I_t
        dR_expected = self.gamma * I_t
    ...
    def get_basic_reproduction_number(
        ...
        """
        Compute effective reproduction number R_t = beta_t * S_t / (gamma * N).

        Args:
            beta_t: [batch_size, horizon]
            S_trajectory: [batch_size, horizon+1] (includes initial state)
            population: [batch_size]

        Returns:
            R_t: [batch_size, horizon] effective reproduction number
        """
        # Use S_t at each step (exclude final trajectory point)
        S_t = S_trajectory[:, :-1]  # [batch_size, horizon]
        N = population.unsqueeze(-1)  # [batch_size, 1]

        R_t = beta_t * S_t / (self.gamma * N)
        return R_t

    def __repr__(self) -> str:
        return (
            f"SIRRollForward(gamma={self.gamma}, dt={self.dt}, "
